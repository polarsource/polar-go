// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package components

import (
	"encoding/json"
	"fmt"
	"github.com/polarsource/polar-go/internal/utils"
	"time"
)

type BenefitDiscordType string

const (
	BenefitDiscordTypeDiscord BenefitDiscordType = "discord"
)

func (e BenefitDiscordType) ToPointer() *BenefitDiscordType {
	return &e
}
func (e *BenefitDiscordType) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "discord":
		*e = BenefitDiscordType(v)
		return nil
	default:
		return fmt.Errorf("invalid value for BenefitDiscordType: %v", v)
	}
}

// BenefitDiscord - A benefit of type `discord`.
//
// Use it to automatically invite your backers to a Discord server.
type BenefitDiscord struct {
	// Creation timestamp of the object.
	CreatedAt time.Time `json:"created_at"`
	// Last modification timestamp of the object.
	ModifiedAt *time.Time `json:"modified_at"`
	// The ID of the benefit.
	ID    string             `json:"id"`
	type_ BenefitDiscordType `const:"discord" json:"type"`
	// The description of the benefit.
	Description string `json:"description"`
	// Whether the benefit is selectable when creating a product.
	Selectable bool `json:"selectable"`
	// Whether the benefit is deletable.
	Deletable bool `json:"deletable"`
	// The ID of the organization owning the benefit.
	OrganizationID string `json:"organization_id"`
	// Properties for a benefit of type `discord`.
	Properties BenefitDiscordProperties `json:"properties"`
}

func (b BenefitDiscord) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(b, "", false)
}

func (b *BenefitDiscord) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &b, "", false, true); err != nil {
		return err
	}
	return nil
}

func (o *BenefitDiscord) GetCreatedAt() time.Time {
	if o == nil {
		return time.Time{}
	}
	return o.CreatedAt
}

func (o *BenefitDiscord) GetModifiedAt() *time.Time {
	if o == nil {
		return nil
	}
	return o.ModifiedAt
}

func (o *BenefitDiscord) GetID() string {
	if o == nil {
		return ""
	}
	return o.ID
}

func (o *BenefitDiscord) GetType() BenefitDiscordType {
	return BenefitDiscordTypeDiscord
}

func (o *BenefitDiscord) GetDescription() string {
	if o == nil {
		return ""
	}
	return o.Description
}

func (o *BenefitDiscord) GetSelectable() bool {
	if o == nil {
		return false
	}
	return o.Selectable
}

func (o *BenefitDiscord) GetDeletable() bool {
	if o == nil {
		return false
	}
	return o.Deletable
}

func (o *BenefitDiscord) GetOrganizationID() string {
	if o == nil {
		return ""
	}
	return o.OrganizationID
}

func (o *BenefitDiscord) GetProperties() BenefitDiscordProperties {
	if o == nil {
		return BenefitDiscordProperties{}
	}
	return o.Properties
}

...
const (
	...
	FileReadTypeSupportCaseAttachment FileReadType = "support_case_attachment"
)
...
type FileRead struct {
	...
	SupportCaseAttachmentFileRead *SupportCaseAttachmentFileRead `queryParam:"inline" union:"member"`
...
func CreateFileReadSupportCaseAttachment(supportCaseAttachment SupportCaseAttachmentFileRead) FileRead {
	typ := FileReadTypeSupportCaseAttachment

	return FileRead{
		SupportCaseAttachmentFileRead: &supportCaseAttachment,
		Type:                          typ,
	}
}
...
func (u *FileRead) UnmarshalJSON(data []byte) error {
	...
	case "support_case_attachment":
		supportCaseAttachmentFileRead := new(SupportCaseAttachmentFileRead)
		if err := utils.UnmarshalJSON(data, &supportCaseAttachmentFileRead, "", true, nil); err != nil {
			return fmt.Errorf("could not unmarshal `%s` into expected (Service == support_case_attachment) type SupportCaseAttachmentFileRead within FileRead: %w", string(data), err)
		}

		u.SupportCaseAttachmentFileRead = supportCaseAttachmentFileRead
		u.Type = FileReadTypeSupportCaseAttachment
		return nil
	}
...
func (u FileRead) MarshalJSON() ([]byte, error) {
	...
	if u.SupportCaseAttachmentFileRead != nil {
		return utils.MarshalJSON(u.SupportCaseAttachmentFileRead, "", true)
	}

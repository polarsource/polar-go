...
func TestUserOrderGet(t *testing.T) {
	...
	_, err := client.Users.Orders.Get(context.TODO(), "id")
	if err != nil {
...
func TestUserOrderListWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Users.Orders.List(context.TODO(), polar.UserOrderListParams{
		Limit:            polar.F(int64(1)),
		OrganizationID:   polar.F[polar.UserOrderListParamsOrganizationIDUnion](shared.UnionString("string")),
...
func TestUserOrderInvoice(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Users.Orders.Invoice(context.TODO(), "id")
	if err != nil {
		var apierr *polar.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))

...
```go
fileCreate := components.CreateFileCreateSupportCaseAttachment(components.SupportCaseAttachmentFileCreate{/* values here */})
```
...
switch fileCreate.Type {
	...
	case components.FileCreateTypeSupportCaseAttachment:
		// fileCreate.SupportCaseAttachmentFileCreate is populated
}
```

...
func TestAccountDashboardLink(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Accounts.DashboardLink(context.TODO(), "182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e")
	if err != nil {
		var apierr *polar.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestAccountOnboardingLink(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Accounts.OnboardingLink(
		context.TODO(),
		"182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e",
		polar.AccountOnboardingLinkParams{
			ReturnPath: polar.F("return_path"),
		},
	)
	if err != nil {
		var apierr *polar.Error
...
func TestAccountSearchWithOptionalParams(t *testing.T) {
	...
	_, err := client.Accounts.Search(context.TODO(), polar.AccountSearchParams{
